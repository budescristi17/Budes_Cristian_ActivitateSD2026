...
void stergeMasiniDupaNumarUsi(ListaDubla* lista, int nrUsi) {
	Nod* p = lista->prim;
	while (p) {
		Nod* urmator = p->next;
		if (p->info.nrUsi == nrUsi) {
			if (p->prev) {
				p->prev->next = p->next;
			}
			else {
...
int main() {
	...
	adaugaMasinaInLista(&lista, initializareMasina(3, 4, 18000, "Dacia", "Maria", 'A'));
	adaugaMasinaInLista(&lista, initializareMasina(4, 5, 16000, "Ford", "Vlad", 'C'));

	stergeMasiniDupaNumarUsi(&lista, 4);
	afisareLista(lista);
