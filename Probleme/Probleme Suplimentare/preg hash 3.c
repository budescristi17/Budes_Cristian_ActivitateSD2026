...
Comanda getComandaDupaId(HashTable ht, unsigned int idComanda) {
	int pozitie = calculeazaHash(idComanda, ht.dim);
	Nod* p = ht.vector[pozitie];
	while (p) {
		if (p->info.idComanda == idComanda) {
			return copiazaComanda(p->info);
		}
		p = p->next;
	}
	Comanda c;
...
int main() {
	...
	inserareComandaInTabela(ht, initializareComanda(3, "12.04.2026", "18.04.2026", "Maria Stan", 'A'));

	Comanda cautata = getComandaDupaId(ht, 2);
	if (cautata.idComanda != 0) {
		afisareComanda(cautata);
		dezalocareComanda(&cautata);
	}
