...
int numaraMuchiiIntreAceeasiSerie(NodP* graf) {
	int contor = 0;
	NodP* p = graf;
	while (p) {
		NodS* vecin = p->vecini;
		while (vecin) {
			if (p->info.serie == vecin->info->info.serie && p->info.id < vecin->info->info.id) {
				contor++;
			}
			vecin = vecin->next;
		}
...
int main() {
	...
	inserareInListaP(&graf, initializareMasina(2, 2, 25000, "BMW", "Ana", 'A'));
	inserareInListaP(&graf, initializareMasina(3, 5, 18000, "Dacia", "Maria", 'C'));
	inserareInListaP(&graf, initializareMasina(4, 4, 12000, "Seat", "Vlad", 'A'));

	inserareMuchie(graf, 1, 2);
	inserareMuchie(graf, 1, 3);
	inserareMuchie(graf, 2, 4);

	printf("Muchii intre masini din aceeasi serie: %d\n", numaraMuchiiIntreAceeasiSerie(graf));
