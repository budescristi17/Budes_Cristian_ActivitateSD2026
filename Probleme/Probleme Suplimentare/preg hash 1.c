...
void inserareComandaInTabela(HashTable ht, Comanda comanda) {
	int pozitie = calculeazaHash(comanda.numeClient, ht.dim);
	adaugaComandaInLista(&ht.vector[pozitie], comanda);
}

int numaraComenziClient(HashTable ht, const char* numeClient) {
	int pozitie = calculeazaHash(numeClient, ht.dim);
	int contor = 0;
...
int main() {
	...
	inserareComandaInTabela(ht, initializareComanda(3, "12.04.2026", "18.04.2026", "Ion Popescu", 'A'));

	printf("Comenzi Ion Popescu: %d\n", numaraComenziClient(ht, "Ion Popescu"));
